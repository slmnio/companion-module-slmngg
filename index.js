...
class instance extends InstanceBase {
    ...
    async init(config) {
        ...
        this.timerTick = setInterval(() => {
            ...
            if (end && endTime) {
                if (diff > 60 * 60) {
                    ...
                    this.setState(
                        "broadcast_countdown_seconds_text",
                        [hours, mins.padStart(2, "0")].join("h")
                    );
                    this.setState("broadcast_countdown_active", true);
                    this.setState("broadcast_countdown_needs_clear", false);
                } else if (diff > 0) {
                    this.setState("broadcast_countdown_seconds", diff);
                    let [mins, secs] = [Math.floor(diff / 60), diff % 60].map(
        ...
        this.config.dataServerHost = this.config.useLocal
            ? this.config.localHost
            : this.config.host;
        this.config.dataServerPort = this.config.useLocal
            ? this.config.localPort
            : this.config.port;
        this.config.dataServerAddress = [
            this.config.dataServerHost,
            this.config.dataServerPort
        ].join(":");
        ...
        this.timerVersionCheck = setInterval(() => {
            this.checkForNewVersion().catch((err) =>
                this.log(
                    "debug",
                    `Scheduled version check failed: ${err?.message || err}`
                )
            );
        }, 10 * 60 * 1000);
    }
    ...
    setState(key, val) {
        ...
        const feedbacksToCheck = Object.entries(this._feedbacks)
            .filter(([feedbackId, feedback]) => feedback?.check?.includes(key))
            .map(([feedbackId, feedback]) => feedbackId);
    ...
    async connectDataServer() {
        let host = this.config.useLocal
            ? this.config.localHost
            : this.config.host;
        let port = this.config.useLocal
            ? this.config.localPort
            : this.config.port;
        ...
        if (!host || !port) {
            this.updateStatus(
                ...
                "No SLMN.GG connection details listed"
            );
        ...
        this.socket.on("connect", () => {
            ...
            this.log(
                "info",
                `Connected to SLMN.GG Data Server (client: ${this.config.clientKey})`
            );

            this.socket.emit("prod-join", this.config.clientKey);
            this.socket.emit("subscribe-multiple", [
                ...this.slmnggCache.keys()
            ]);
        ...
        this.socket.on("data_update", (id, data) => {
            ...
            if (
                ...
                data.__tableName === "Clients"
            )
                this.slmnggCache.set("client", data);
        ...
        const sceneTargetsMyRoles = (_sceneName) => {
            ...
            if (
                ["OBSDIR", "Director", "Clean feed"].some((str) =>
            ...
            const observerNumber = this.states
                .get("staff_observer_number")
                ?.toString();
            if (
                observerNumber &&
                ["Obs", "Game"].some((str) =>
        ...
        this.socket.on("prod_preview_program_change", (data) => {
            ...
            if (clientPositions.includes("Producer") && sceneTargetsMyRoles(programScene)) {
                return this.setState("observer_tally", "program");
            }

            if (clientPositions.includes("Observer Director") && sceneTargetsMyRoles(programScene)) {

                const producer = clientPositions.includes("Producer") && !clientPositions.includes("Observer Director");
            ...
            if(clientPositions.includes("Producer") && sceneTargetsMyRoles(previewScene)) {
            ...
            if (clientPositions.includes("Observer Director") && sceneTargetsMyRoles(previewScene)) {
        ...
        this.socket.on(
            "action_error",
            ({ action, error, errorCode, errorMessage }) => {
                this.log(
                    "error",
                    `Action error on [${action}]: ${errorCode} - ${JSON.stringify(
                        errorMessage
                    )}`
                );
            }
        ...
        this.socket.on("connect_error", (error) =>
            this.log(
                "error",
                `Socket connection error: ${error.message} ${JSON.stringify(
                    error
                )}`
            )
        );
        ...
        this.socket.io.on("reconnect", (attempt) =>
            this.log("info", `Socket reconnected after ${attempt} attempt(s)`)
        );
        this.socket.io.on("reconnect_error", (error) =>
            this.log("info", `Socket reconnection error ${error.message}`)
        );
        this.socket.io.on("reconnect_failed", () =>
            this.log("info", `Socket reconnection failed`)
        );
        this.socket.io.on("reconnect_attempt", (attempt) =>
            this.log("debug", `Socket reconnecting: attempt ${attempt}`)
        );
    }
    ...
    async generateData() {
        ...
        if (await this.getData(`client-${this.config.clientKey}`)) {
            let client = this.slmnggCache.get(
            ...
            if (client?.key && streams?.length) {
                ...
                const myStreams = streams.filter(
                    (s) => s.clientName === client?.key
                );
                // client_stream_delay
                const delay = myStreams.find(
                    (s) =>
                ...
                if (delay !== undefined) {
                    ...
                    this.setState(
                        `client_stream_delay_ms`,
                        parseInt(delay) * 1000
                    );
                }
            }

            if (client.broadcast?.length) {
    ...
    async checkForNewVersion() {
        const res = await fetch(
            "https://ungh.cc/repos/slmnio/companion-module-slmngg/releases/latest"
        );
        if (!res.ok) {
    ...
    async generateBroadcast(broadcast) {
        ...
        this.setState(
            "broadcast_observer_settings",
            (broadcast.observer_settings || []).join(", ")
        );
        ...
        if (broadcast.event?.length) {
            ...
            if (event.theme?.[0]) {
                ...
                if (theme) {
                    ...
                    this.setState(
                        "broadcast_event_theme_logo_background",
                        theme.color_logo_background
                    );
                    this.setState(
                        "broadcast_event_theme_text_on_logo_background",
                        theme.color_text_on_logo_background
                    );
                    this.setState(
                        "broadcast_event_theme_color",
                        theme.color_theme
                    );
                    this.setState(
                        "broadcast_event_theme_text_on_theme",
                        theme.color_text_on_theme
                    );
                }
            } else {
                // unset theme
    ...
    generateMatch(match) {
        ...
        this.setState(
            "match_is_complete",
            match.score_1 === match.first_to || match.score_2 === match.first_to
        );
    ...
    async generateTeams(match) {
        if (!match?.teams) {
            ...
            [
                ...
                [1, 2].forEach((teamNum) => {
                    this.setState(
                        `team_${teamNum}_${key}`,
                        key === "theme_ready" ? false : ""
                    );
                });
                ["left", "right"].forEach((teamSide) => {
                    this.setState(
                        `team_${teamSide}_${key}`,
                        key === "theme_ready" ? false : ""
                    );
                });
            });
        ...
        for (let i = 0; i < match.teams.length; i++) {
            ...
            let teamNum = i + 1;
            let teamSide = this.states.get("match_flip_teams")
                ? ["right", "left"][i]
                : ["left", "right"][i];
                ...
                if (team.players?.length) {
                    (team.players || []).forEach(async (playerID, pi) => {
                        ...
                        const setPlayerState = (key, val) => {
                            this.setState(
                                `player_team_${num}_player_${pi + 1}_${key}`,
                                val
                            );
                            this.setState(
                                `player_team_${teamSide}_player_${
                                    pi + 1
                                }_${key}`,
                                val
                            );
                    ...
                    [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach((playerNum) => {
                        ...
                        const setPlayerState = (key, val) => {
                            this.setState(
                                `player_team_${num}_player_${playerNum}_${key}`,
                                val
                            );
                            this.setState(
                                `player_team_${teamSide}_player_${playerNum}_${key}`,
                                val
                            );
                        };
                        setPlayerState("id", "");
    ...
    async generateRelationships(match) {
        ...
        for (const relID of match.player_relationships) {
            ...
            if (relationship) {
                ...
                if (staff) {
                    ...
                    if (
                        staff.id === this.states.get("client_staff") &&
                        relationship.singular_name === "Observer"
                    ) {
        ...
        Object.entries(rels).forEach(([singularName, relationships]) => {
            [0, 1, 2, 3, 4, 5].forEach((i) => {
                ...
                let stateKey = `staff_${singularName
                    .toLowerCase()
                    .replace(/ /g, "_")}`;

                if (rel?.player?.name) {
                    const name = rel.player.short
                        ? rel.player.short?.[0]
                        : rel.player.name;
                    const code =
                        rel.player?.client_key || rel.player.name.toLowerCase();
